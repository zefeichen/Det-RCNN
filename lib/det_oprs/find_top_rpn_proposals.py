...
from lib.det_oprs.box_ops import loc2bbox, clip_boxes_opr, filter_boxes_opr

@torch.no_grad()
def find_top_rpn_proposals(rpn_bbox_offsets_list, rpn_cls_prob_list, all_anchors_list, im_info, training, args):
    prev_nms_top_n = args.train_prev_nms_top_n if training else args.test_prev_nms_top_n
    post_nms_top_n = args.train_post_nms_top_n if training else args.test_post_nms_top_n

    batch_per_gpu = len(im_info)
    nms_threshold = args.rpn_nms_threshold
    box_min_size = args.rpn_min_box_size
    ...
    for bid in range(batch_per_gpu):
        ...
        for l in range(list_size):
            ...
            proposals = loc2bbox(all_anchors, offsets)
            if args.anchor_within_border:
        ...
        # if (not training) and len(batch_proposals) < post_nms_top_n:
        #     save_index = torch.multinomial(torch.ones(len(batch_proposals)), num_samples = post_nms_top_n, replacement = True)
        #     batch_proposals = batch_proposals[save_index]
    ...
    return concated_rois, batch_probs
