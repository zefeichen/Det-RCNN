...
class COCOMReval:
    ...
    def summarize(self,id_setup):
        ...
        def _summarize(iouThr=None, maxDets=100 ):
            ...
            result_info = iStr.format(titleStr, typeStr,setupStr, iouStr, heightStr, occlStr, f_heightStr, mean_s*100)
            print(result_info)
...
def compute_MMR(json_result_file, anno_file, record_file):
    mr_res_dict = OrderedDict()
    iou_type="bbox"
    id_c = 0
    mr_mode = "crowdhuman"
    for id_setup in range(3,4):
        ...
        imgIds = sorted(coco_mod_gt.getImgIds())
        # imgIds = sorted(iduse)
        coco_mr_eval = COCOMReval(coco_mod_gt, coco_mod_dt, iou_type, record_file)
        coco_mr_eval.params.imgIds = imgIds
        coco_mr_eval.evaluate(id_setup)
        coco_mr_eval.accumulate()      
        mean_s = coco_mr_eval.summarize(id_setup) 
