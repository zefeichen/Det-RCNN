...
from lib.util.misc import NestedTensor
...
def build_position_encoding(args):
    n_steps = args.deformable_detr.hidden_dim // 2
    sine_emedding_func = PositionEmbeddingSine
    if args.deformable_detr.backbone.position_embedding in ('v2', 'sine'):
        # TODO find a better way of exposing other arguments
        position_embedding = sine_emedding_func(n_steps, normalize=True)
    elif args.deformable_detr.backbone.position_embedding in ('v3', 'learned'):
        position_embedding = PositionEmbeddingLearned(n_steps)
    else:
        raise ValueError(f"not supported {args.deformable_detr.position_embedding}")
